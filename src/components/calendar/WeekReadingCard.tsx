...
function sessionKey(readingId: string, weekStart: string) {
  return `week_interp_${readingId}_${weekStart}`;
}
...
export default function WeekReadingCard({ weekStart, readingId, isPremium }: Props) {
  ...
  const [content,  setContent]  = useState<string | null>(() => {
    ...
    return sessionStorage.getItem(sessionKey(readingId, weekStart));
  });
  ...
  useEffect(() => {
    const key = sessionKey(readingId, weekStart);
    ...
    fetch("/api/week-interpretation", {
      ...
      body: JSON.stringify({ week_start: weekStart, reading_id: readingId, check_only: true }),
    })
      .then(async r => {
        if (r.ok) {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setContent(c);
      sessionStorage.setItem(sessionKey(readingId, weekStart), c);
    } catch (e) {
