...
export default function HeroSky() {
  ...
  const stateRef   = useRef<{
    planets: Array<{ a: number; mult: number; hov: boolean; fade: number; el: HTMLElement; orbitEl: SVGEllipseElement }>;
    stars: Array<{ x: number; y: number; r: number; layer: number; tw: number; warm: boolean }>;
    W: number; H: number;
  ...
  useEffect(() => {
    ...
    // ── Build orbit SVG ellipses + planet DOM nodes ──
    ...
    const planetState = PLANETS.map((p, i) => {
      ...
      e.style.transition = "stroke .3s, opacity .3s";
      orb.appendChild(e);

      const d = document.createElement("div");
      d.className = "lp";
      d.style.cssText = `position:absolute;border-radius:50%;z-index:3;cursor:pointer;
        width:${p.r*2}px;height:${p.r*2}px;
        background:radial-gradient(circle at 35% 35%, #fff6e8 0%, ${p.c} 45%, ${p.c} 100%);
        box-shadow:0 0 ${p.r*2.6}px ${p.c}66;transition:filter .3s`;
      if ("ring" in p && p.ring) {
      ...
      d.addEventListener("mouseenter", () => {
        ...
        st.hov = true;
        e.setAttribute("stroke", "var(--accent-deep)");
        e.style.opacity = "0.9";
        if (tip) { tip.innerHTML = `<b>${p.s}︎</b> ${p.n}`; tip.style.opacity = "1"; tip.style.transform = "translateY(0)"; }
      });
    ...
    let stars: typeof stateRef.current extends null ? never : NonNullable<typeof stateRef.current>["stars"] = [];
    let W = 0, H = 0;
    ...
    function sizeCanvas() {
      ...
      for (let gx = 0; gx < COLS; gx++) {
        for (let gy = 0; gy < ROWS; gy++) {
          ...
          stars.push({
            x, y,
            r: (Math.random() * 1.1 + 0.45) * devicePixelRatio,
            layer: (gx + gy) % 2,
    ...
    function computeZones() {
      ...
      zones = [
        { x1: 0, y1: 0, x2: hero!.offsetWidth, y2: 120 },
        { x1: c.left - h.left - 48, y1: c.top - h.top - 40, x2: c.right - h.left + 48, y2: c.bottom - h.top + 40 },
      ];
    }
    computeZones();
    ...
    function frame(now: number) {
      ...
      const scale = window.innerWidth < 768 ? 0.52 : window.innerWidth < 1100 ? 0.75 : 1;

      if (sky) sky.style.transform = `translate(${px * 16}px, ${py * 16}px)`;
      ...
      planetState.forEach((st, i) => {
        ...
        st.el.style.pointerEvents = st.fade < 0.6 ? "none" : "auto";
    ...
    function staticSky() {
      const scale = window.innerWidth < 768 ? 0.52 : 1;
      if (orb) orb.style.transform = `translate(-50%, -50%) scale(${scale})`;
      planetState.forEach((st, i) => {
        const p = PLANETS[i];
    ...
    if (cta && !reduced) {
      ...
      cta.addEventListener("mousemove", (e: Event) => {
        ...
        cta.style.transform = `translate(${dx * 4}px, ${dy * 4}px)`;
      });
  ...
  return (
    ...
    >
      ...
      {/* planets + orbits */}
      <div ref={skyRef} style={{ position: "absolute", inset: 0, zIndex: 2, willChange: "transform" }}>
        <svg
          ref={orbitsRef}
          id="orbits"
          width="1400"
      ...
      >
        ...
        <h1 style={{ fontSize: "clamp(40px,6.5vw,72px)", fontWeight: 700, lineHeight: 1.05, letterSpacing: "-0.02em", marginBottom: 28 }}>
        ...
        <p className="landing-reveal landing-d3" style={{ fontSize: "clamp(17px,1.6vw,19.5px)", lineHeight: 1.6, color: "var(--text-secondary)", maxWidth: 660, margin: "0 auto 44px" }}>
        ...
        <div className="landing-reveal landing-d4">
          <Link
            ...
            style={{
              ...
              padding: "19px 40px", textDecoration: "none",
              boxShadow: "0 0 48px rgba(255,174,61,.18)",
      ...
      <style>{`
        ...
        .landing-d1 { animation-delay: .35s; }
        .landing-d2 { animation-delay: .5s; }
        .landing-d3 { animation-delay: .65s; }
        .landing-d4 { animation-delay: .8s; }
        ...
        @media (max-width:768px) { #orbits ellipse { display: none; } }
      `}</style>
    </header>
  );
}
