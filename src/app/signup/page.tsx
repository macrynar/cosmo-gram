...
function MysticInput({
  type, placeholder, value, onChange, icon: Icon, minLength,
}: {
          ...
          {info ? (
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <MysticInput type="password" placeholder="min. 6 znaków" value={password}
                  onChange={e => setPassword(e.target.value)} icon={Lock} minLength={6} />
              ...
              <p className="mt-5 text-center text-xs text-slate-600">
                Masz już konto?{" "}
                <Link
                  href={`${ROUTES.public.login.path}${redirectTo !== ROUTES.app.today.path ? `?redirect=${encodeURIComponent(redirectTo)}` : ""}`}
