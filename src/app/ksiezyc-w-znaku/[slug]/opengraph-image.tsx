...
import { getMoonSign } from "@/lib/pseo/moonSigns";
import { SIGN_LOCATIVE } from "@/lib/i18n/astro";

export const runtime = "nodejs";
export const size = { width: 1200, height: 630 };
export const contentType = "image/png";
export const alt = "Cosmogram — Księżyc w znaku";
...
/** Pobiera font z Google Fonts jako ArrayBuffer(y); woff (satori nie obsługuje woff2),
 * subsety latin + latin-ext (polskie znaki). Cichy fallback do []. */
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const loc = m ? SIGN_LOCATIVE[m.sign] ?? m.sign : "";
  const title = m ? `Księżyc w ${loc}` : "Cosmogram";
